...
createScotStickerButton();
  ...
  void draw() {
    ...
    if (cam.available()){
      //10, 940, 10
      //10, 580, 10
      prev.copy(cam, 0,0, 940, 580, 0, 0, 940, 580);
      prev.updatePixels();
      cam.read();
  }
  ...
  if(((Button)(cp5.getController("P3"))).isOn()){
    ...
    for (int y = 0; y < 580; y++ ) {
      ...
      color currentColor = processedImage.pixels[loc];
      ...
      if (difference > threshold*threshold) {
        processedImage.pixels[loc] = color(0,0,0);
      } else {
        processedImage.pixels[loc] = color(255,255,255);
      }
